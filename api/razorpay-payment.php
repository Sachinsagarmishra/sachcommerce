                    ...
                    <?php if (!$is_razorpay_configured): ?>
                        <div class="alert alert-warning text-start">
                            ...
                            <?php else: ?>
                                ...
                                <p class="small text-muted mt-1">Please go to Admin Panel > Settings > Payment Settings to configure your API keys.</p>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="d-grid gap-3 mt-4">
                        ...
                        <?php else: ?>
                            <button class="btn btn-secondary btn-lg" disabled>
                                <i class="fas fa-lock me-2"></i>Payment Unavailable
                            </button>
                        <?php endif; ?>
                        ...
                        <a href="<?php echo SITE_URL; ?>/payment-failed.php?order=<?php echo $order['order_number']; ?>" class="btn btn-link text-danger text-decoration-none">
                            Cancel Payment
                        </a>
                    </div>
                </div>
                ...
                "handler": function (response){
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            if (result.success) {
                                window.location.href = '<?php echo SITE_URL; ?>/payment-success.php?order=<?php echo $order['order_number']; ?>';
                            } else {
                                alert(result.message || 'Payment verification failed');
                                window.location.href = '<?php echo SITE_URL; ?>/payment-failed.php?order=<?php echo $order['order_number']; ?>';
                            }
                        },
                        error: function() {
                            alert('Network error verifying payment');
                            $('#rzp-button').html('<i class="fas fa-lock me-2"></i>Pay Now Securely');
