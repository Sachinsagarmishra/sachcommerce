...
<head>
    ...
    <!-- Third Party Scripts -->
    <?php if ($site_settings['google_search_console']) echo $site_settings['google_search_console']; ?>
    <?php if ($site_settings['google_tag_manager_id']): ?>
        <script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','<?php echo $site_settings['google_tag_manager_id']; ?>');</script>
    <?php endif; ?>
    <?php if ($site_settings['google_analytics_id']): ?>
        ...
        <script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','<?php echo $site_settings['google_analytics_id']; ?>');</script>
    <?php endif; ?>
    <?php if ($site_settings['facebook_pixel_id']): ?>
        <script>!function(f,b,e,v,n,t,s){if(f.fbq)return;n=f.fbq=function(){n.callMethod?n.callMethod.apply(n,arguments):n.queue.push(arguments)};if(!f._fbq)f._fbq=n;n.push=n;n.loaded=!0;n.version='2.0';n.queue=[];t=b.createElement(e);t.async=!0;t.src=v;s=b.getElementsByTagName(e)[0];s.parentNode.insertBefore(t,s)}(window,document,'script','https://connect.facebook.net/en_US/fbevents.js');fbq('init','<?php echo $site_settings['facebook_pixel_id']; ?>');fbq('track','PageView');</script>
    <?php endif; ?>
    <?php if ($site_settings['microsoft_clarity_id']): ?>
        <script type="text/javascript">(function(c,l,a,r,i,t,y){c[a]=c[a]||function(){(c[a].q=c[a].q||[]).push(arguments)};t=l.createElement(r);t.async=1;t.src="https://www.clarity.ms/tag/"+i;y=l.getElementsByTagName(r)[0];y.parentNode.insertBefore(t,y);})(window,document,"clarity","script","<?php echo $site_settings['microsoft_clarity_id']; ?>");</script>
    <?php endif; ?>
    <?php if ($site_settings['custom_header_scripts']) echo $site_settings['custom_header_scripts']; ?>
    ...
    <style>
        ...
        body { font-family: <?php echo $body_font; ?>; color: <?php echo $body_text_color; ?>; }
        h1, h2, h3, h4, h5, h6, .section-title, .navbar-brand { font-family: <?php echo $heading_font; ?>; color: <?php echo $heading_text_color; ?>; }

        /* Navbar */
        .navbar { background-color: <?php echo $navbar_bg; ?>; }
        .navbar-nav .nav-link { color: <?php echo $nav_link_color; ?>; }
        .navbar-nav .nav-link:hover { color: <?php echo $nav_link_hover_color; ?>; }
        .navbar-brand { color: <?php echo $nav_link_color; ?>; }

        /* Product Cards & Prices */
        .product-title, .product-title a { color: <?php echo $product_title_color; ?>; }
        .product-price, .selling-price, .product-price-detail { color: <?php echo $primary_color; ?> !important; }
        
        .badge-sale, .badge-discount { background-color: <?php echo $sale_badge_bg; ?>; }
        .badge-new { background-color: <?php echo $new_badge_bg; ?>; }
        
        /* Buttons */
        .btn-primary, .add-to-cart-btn, .btn-orange-custom {
        ...
        .btn-primary:hover, .add-to-cart-btn:hover, .btn-orange-custom:hover { opacity: 0.9; }

        .btn-outline-primary {
            color: <?php echo $btn_primary_bg; ?>;
            border-color: <?php echo $btn_primary_bg; ?>;
        }
        .btn-outline-primary:hover {
            background-color: <?php echo $btn_primary_bg; ?>;
            color: <?php echo $btn_primary_text; ?>;
        }

        /* Footer */
        .footer { background-color: <?php echo $footer_bg; ?>; }
        .footer h4, .footer h5, .footer .footer-title, .footer .widget-title { color: <?php echo $footer_heading_color; ?>; }
        .footer p, .footer span, .footer div, .footer li, .footer small { color: <?php echo $footer_text_color; ?>; }
        .footer a { color: <?php echo $footer_text_color; ?>; opacity: 0.8; text-decoration: none; }
        .footer a:hover { color: <?php echo $primary_color; ?>; opacity: 1; }
    </style>
